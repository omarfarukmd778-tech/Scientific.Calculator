...
public class Scientific_calculator extends JFrame {
        ...
        if (!label.isEmpty()) {
            b.addMouseListener(new MouseAdapter() {
                public void mouseEntered(MouseEvent e) {
                    if (isOn) b.setBackground(bg.brighter());
                }
    ...
    private void applyUnary(String cmd) {
        ...
        switch (cmd) {
            ...
            case "sin":
                result = Math.sin(Math.toRadians(v));
                break;

            case "cos":
                result = Math.cos(Math.toRadians(v));
                break;

            case "tan":
                if (Math.abs(Math.cos(Math.toRadians(v))) < 1e-10) {
                    errVal("Undefined");
                    return;
                }
                result = Math.tan(Math.toRadians(v));
                break;
        ...
        resultDone = true;
        newInput = false;
        statusLabel.setText("SCIENTIFIC");
    }
